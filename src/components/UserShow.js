    ...
    userBoardgamesLinks = user.owned_games.map((game) => {
      return (
        <StyledInnerDiv key={game.id}>
          ...
          {user.id === loggedInUser.id ? <StyledButton onClick={(e) => handleRemoveFromShelf(game.id)}>
            Remove From Shelf
          </StyledButton> : null}
        </StyledInnerDiv>
      );
    ...
    <div style={{textAlign: "center"}}>
      ...
      <StyledInnerDiv>
        ...
        <StyledForm onSubmit={(e) => editProfileInfo(e)}>
          ...
          <StyledButton as="input" type="text" value={name} onChange={(e)=>setName(e.target.value)}/>
          <br></br>
          <StyledP as="label">Username: </StyledP>
          <StyledButton as="input" type="text" value={username} onChange={(e)=>setUsername(e.target.value)}/>
          <br></br>
          <StyledP as="label">Email: </StyledP>
          <StyledButton as="input" type="text" value={email} onChange={(e)=>setEmail(e.target.value)}/>
          <br></br>
          <StyledP as="label">Profile Image Link: </StyledP>
          <StyledButton as="input" type="text" value={profilePicture} onChange={(e)=>setProfilePicture(e.target.value)}/>
          <br></br>
        ...
        <>
        <StyledButton onClick={()=>setEditProfile(!editProfile)}>Edit Profile</StyledButton>
        <StyledButton onClick={(e)=>deleteProfile()}>Delete Profile</StyledButton> 
        </>
        : null }
      </StyledInnerDiv>
      <StyledInnerDiv>
        <StyledImg src={user.profile_picture} alt="profile" style={{maxHeight: "25vw", maxWidth: "25vw"}}/>
      </StyledInnerDiv>
      </StyledDiv>
      <br></br>
      <h3>Game Shelf</h3>
